...
router.post("/:id", async (req, res) => {
    ...
    const [dupcheck] = await knex("bookmark").select("*").where({ type: req.body.type, user_id: req.params.id })
    console.log(dupcheck);
    const trx = await knex.transaction();
    const type = req.body.type == 0 ? "talk" : "think";
    const num_name = type + "_num";
    const [brf_bookmark] = await knex(type).select("bookmark").where(num_name, req.params.id);
    console.log(dupcheck);
