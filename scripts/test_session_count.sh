...
echo
echo "=== GET /admin/session_count ==="
body=$(curl -s "$BASE_URL/admin/session_count" -b "$COOKIE_FILE")

if echo "$body" | grep -q "일별 유저별 세션 횟수"; then
    echo "[PASS] 페이지 정상 응답"
else
    echo "[FAIL] 예상한 제목을 찾지 못했습니다. (MySQL 8.0 미만이면 WITH/LAG 구문 에러일 수 있음) 응답 일부:"
